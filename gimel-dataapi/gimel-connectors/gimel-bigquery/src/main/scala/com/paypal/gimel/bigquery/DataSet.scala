...
class DataSet(sparkSession: SparkSession) extends GimelDataSet(sparkSession: SparkSession) {
  ...
  override def read(dataset: String, dataSetProps: Map[String, Any]): DataFrame = {
    ...
    if (dataSetProps.isEmpty) {
      throw new Exception("Props Map Cannot be empty for KafkaDataSet Read.")
    }
    ...
    try {
      ...
      case e: Throwable =>
        val msg = s"Error reading from Big Query : ${e.getMessage}"
  ...
  override def write(dataset: String, dataFrame: DataFrame, dataSetProps: Map[String, Any]): DataFrame = {
    ...
    val options: Map[String, String] = datasetProps.props
    try {
      val table: Option[String] = options.get(BigQueryConstants.table)
      table match {
        case None => throw new Exception("Big Query Requires Table Name. Please pass the option [table] in the write API")
      }
      ...
      dfWriter.save(table.get)
      dataFrame
    } catch {
      case e: Throwable =>
        val msg = s"Error writing into Big Query  : ${e.getMessage}"
        throw new DataSetOperationException(msg, e)
    }
  }
