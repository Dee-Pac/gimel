...
export final_jar=${standalone_dir}/lib/gimel-tools-1.2.0-SNAPSHOT-uber.jar
...
check_error $? 999 "Bootstraping hbase tables - failed"

write_log "Bootstraping Kafka topic"
sh ${standalone_dir}/bin/bootstrap_kafka.sh
check_error $? 999 "Bootstraping kafka topic - failed"
...
docker exec -it spark-master bash -c "export USER=$USER; export SPARK_HOME=/spark/; /spark/bin/spark-shell --jars /root/gimel-tools-1.2.0-SNAPSHOT-uber.jar"
